...
def app_device_fault(msg_body_dict: dict):
    ...
    if HStategrid.get_flaut_status():
        ...
        for i in range(0, HStategrid.get_DeviceInfo("00110") + 1):
            fault_warn_code.update({i: []})  # {"device_num":[]}
        if warnSum < 0 or faultSum < 0:
            print("\033[91m故障告警信息错误\033[0m")
            return -1
        elif warnSum > 0 or faultSum > 0:
            for info in faultVal:
                for device_type, device_data in HHhdlist.flaut_warning_type.items():
                    for flaut_type, flaut_data in device_data.items():  # flaut_type:flaut
                        if info.get("fault_id") in flaut_data:
                            if flaut_type in HHhdlist.flaut_warning_type["device"].keys():
                                if flaut_type not in fault_warn_code[0]:
                                    fault_warn_code[0].append(hex(flaut_type))

                            if flaut_type in HHhdlist.flaut_warning_type["gun"].keys():
                                if flaut_type not in fault_warn_code[info.get("device_num")]:
                                    fault_warn_code[info.get("device_num")].append(hex(flaut_type))

            for info in warnVal:
                for device_type, device_data in HHhdlist.flaut_warning_type.items():  # device_type:device
                    for flaut_type, flaut_data in device_data.items():  # flaut_type:flaut
                        if info.get("fault_id") in flaut_data:
                            if flaut_type in HHhdlist.flaut_warning_type["device"].keys():
                                if flaut_type in HHhdlist.flaut_warning_type["device"].keys():
        ...
        if HStategrid.fault_warn_code == {}:
            HStategrid.fault_warn_code = fault_warn_code
            for device, device_data in HStategrid.fault_warn_code.items():
                for fault_id in device_data:
                    info = {
                        "ChargeID": "gun_id",
                        "device": device,
                        "faultid": fault_id,
            ...
            for device_type, fault_id in fault_warn_code.items():
                for old_device_type, old_fault_id in HStategrid.fault_warn_code.items():
                    if device_type == old_device_type:
                        fault_list = set(fault_id) - set(old_fault_id)
                        for fault in fault_list:
                            ...
                            HTools.Htool_send_totalFaultEvt(info)
                        old_fault_list = set(old_fault_id) - set(fault_id)
                        for old_fault in old_fault_list:
                            info = {
                                "ChargeID": "gun_id",
                                "device": device_type,
                                "faultid": old_fault,
